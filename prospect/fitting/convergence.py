...
def find_subsequence(subseq, seq):
	...
	can be modified to return the appropriate index (useful to test WHERE a chain is converged)
	'''
...
def convergence_check(chain, convergence_check_interval=None, convergence_chunks=325,
	...
	:param convergence_nhist:
	    Controls how finely the PDF is subsampled before comparison. This has a strong 
	    effect on the normalization of the KL divergence. Larger --> more noise but 
	    finer distinction between PDF shapes.

	:param convergence_kl_threshold:
	    The convergence criteria for the KL test. Suggest running multiple long 
	    chains and plotting the KL divergence in each parameter to determine how 
	    to set this.
	...
	:returns outdict:
		contains the results of the KL test for each parameter (number of checks, number 
		of parameters) and the iteration where this was calculated.
	"""
	...
	for n in xrange(ncheck):
		for i in xrange(npars):
			...
			early_test_chain = chain[:,(xiter[n]-2*convergence_chunks):(xiter[n]-convergence_chunks),i].flatten()
			...
			late_test_chain = np.clip(chain[:,(xiter[n]-convergence_chunks):xiter[n],i].flatten(), bins[0],bins[-1])
	...
	convergence_flag = find_subsequence([True]*convergence_stable_points_criteria,converged_idx.tolist())
